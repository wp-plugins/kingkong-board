...
<?php
  ...
  if($comment_auto_link == 'T') : $content = preg_replace('/((http|https):\/\/[\w-]+(\.[\w-]+)+([\w.,@?^=%&amp;:\/~+#-]*[\w@?^=%&amp;\/~+#-])?)/', '<a href="\1" target="_blank">\1</a>', $comment->comment_content); endif;
?>
      <div class="comment-content-text">
        <?php
          echo apply_filters('kkb_comment_content_inner_before', null, $board_id, $entry_id, $comment->comment_ID);
          echo "<h2 class='kkb-read-h2'>". nl2br($content)."</h2>";
