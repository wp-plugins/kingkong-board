<?php
  ...
  $kkbContent .= '<form method="get" action="'.get_the_permalink().kingkongboard_set_prefix().'mode=search">';
  ...
  if($pageCount > 1){
    ...
    if( $page > 3){
      $pagenation .= '<a href="'.get_the_permalink().kingkongboard_set_prefix().'pageid=1"><span>1</span></a>';
      ...
      for ($i=($page-3); $i < $pageCount; $i++) { 
        if($page == ($i+1)){
          ...
          if(($i+1) > $page + 2){
            $pagenation .= '<span class="none-select">...</span>';
            $pagenation .= '<a href="'.get_the_permalink().kingkongboard_set_prefix().'pageid='.$pageCount.$param_section.$param_keyword.'"><span>'.$pageCount.'</span></a>';
            break;
          } else {
            $pagenation .= '<a href="'.get_the_permalink().kingkongboard_set_prefix().'pageid='.($i+1).$param_section.$param_keyword.'"><span>'.($i+1).'</span></a>';
          }
        }
      }
    } else {
  ...
  if($permission_write_status == "checked"){
    $kkbContent .= '<a href="'.get_the_permalink($post->ID).kingkongboard_set_prefix().'view=write" class="kingkongboard-button">'.__('글쓰기', 'kingkongboard').'</a>';
  }
  $kkbContent .= '</div>';
