<?php
  ...
  $kkbContent .= '<button type="submit" class="kingkongboard-search-button kingkongboard-button" style="height:26px">검색</button>';
  $kkbContent .= '</td>';
  $kkbContent .= '</tr>';
  $kkbContent .= '</table>';
  $kkbContent .= '</div>';
  $kkbContent .= '</form>';
  $kkbContent .= '</div>';
  $kkbContent .= '<input type="hidden" name="page_permalink" value="'.get_the_permalink($post->ID).'">';
  ...
  if($permission_write_status == "checked"){
    ...
    $kkbContent .= '<a href="'.$write_path.'" class="kingkongboard-button">'.__('글쓰기', 'kingkongboard').'</a>';
  }
  $kkbContent .= '</div>';
