...
if(isset($_POST['view_type']) && isset($_POST['entry_id']) && isset($_POST['board_id'])){
    ...
    switch(sanitize_text_field($_GET['view'])){
        ...
        $verifyContent .= '</form>';
        $verifyContent .= '<div style="margin-top:10px"><a href="'.get_the_permalink().kingkongboard_set_prefix().'pageid='.$pageid.'" class="kingkongboard-button">'.__('목록보기', 'kingkongboard').'</a></div>';
