...
if(isset($_POST['view_type']) && isset($_POST['entry_id']) && isset($_POST['board_id'])){
    ...
    switch(sanitize_text_field($_GET['view'])){
        case "read" :
            ...
            $button     = '<div style="margin-top:10px">'.apply_filters('kkb_verify_read_back_path', '<a href="'.$list_path.'" class="'.kkb_button_classer($board_id).'">'.__('목록보기', 'kingkongboard').'</a>', $pageid, $entry_id).'</div>';
        break;

        case "delete" :
            $input_text = __('삭제 하시려면 ', 'kingkongboard');
            $view_path  = add_query_arg( array('pageid' => $pageid, 'view' => 'read', 'id' => $entry_id), get_the_permalink());
            $button     = '<div style="margin-top:10px">'.apply_filters('kkb_verify_delete_back_path', '<a href="'.$view_path.'" class="'.kkb_button_classer($board_id).'">'.__('돌아가기', 'kingkongboard').'</a>', $pageid, $entry_id).'</div>';
        break;
