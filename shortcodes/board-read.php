<?php
  if(isset($_GET['id'])){
    ...
    $kkbContent .= '<a href="'.$list_path.'" class="'.kkb_button_classer($board_id).'">'.__('목록보기', 'kingkongboard').'</a>';

    if($prevListNumber){
        $prev_path = add_query_arg( array('pageid' => $pageid, 'view' => 'read', 'id' => $prevListNumber), get_the_permalink());
        $kkbContent .= '<a href="'.$prev_path.'" class="'.kkb_button_classer($board_id).'">'.__('이전글', 'kingkongboard').'</a>';
    }
    if($nextListNumber){
        $next_path = add_query_arg( array('pageid' => $pageid, 'view' => 'read', 'id' => $nextListNumber), get_the_permalink());
        $kkbContent .= '<a href="'.$next_path.'" class="'.kkb_button_classer($board_id).'">'.__('다음글', 'kingkongboard').'</a>';
    }
    ...
    if($permission_write_status == "checked" && $board_reply_use == "T"){
        ...
        $kkbContent .= '<a href="'.$reply_path.'" class="'.kkb_button_classer($board_id).'">'.__('답글쓰기', 'kingkongboard').'</a>';
    }
    $kkbContent .= '</div>';
    ...
    if(kingkongboard_button_permission_check($board_id, $entry_id)){
        ...
        $kkbContent .= '<a href="'.$modify_path.'" class="'.kkb_button_classer($board_id).'">'.__('글수정', 'kingkongboard').'</a>';
    }
    ...
    if($permission_delete_status == "checked"){
        ...
        $kkbContent .= '<a class="'.kkb_button_classer($board_id).' button-entry-delete" data="'.$delete_path.'">'.__('글삭제', 'kingkongboard').'</a>';
    }
    $kkbContent .= '</div>';
    $kkbContent .= '</div>';
