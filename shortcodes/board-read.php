<?php
  if(isset($_GET['id'])){
    ...
    $auto_link   = get_post_meta($board_id, 'kkb_auto_link', true);
    $board_read_under   = get_post_meta($board_id, 'kkb_read_under_loop', true);

    $content     = nl2br(get_post_field('post_content', $entry_id));
    if($auto_link == 'T'){
        $content     = preg_replace('/((http|https):\/\/[\w-]+(\.[\w-]+)+([\w.,@?^=%&amp;:\/~+#-]*[\w@?^=%&amp;\/~+#-])?)/', '<a href="\1" target="_blank">\1</a>', $content);
    }
    ...
    if($attached){
        ...
        foreach($attached as $attach){
            ...
            $filetype = get_post_meta($attach, 'file_type', true);
            ...
            $attach_content .= '<div class="entry_each_attach">'.$typeIcon.'<a href="'.KINGKONGBOARD_PLUGINS_URL.'/data/download.php?eid='.$entry_id.'&aid='.$attach.'">'.get_the_title($attach).'.'.$filetype.' <span style="color:gray">'.kingkongboard_attached_getSize($attach).'</span></a></div>';
        }
        $attach_content .= '</td>';
        $attach_content .= '</tr>';
    ...
    if($permission_modify_status == "checked"){
        $Rcontrollers['modify'] = array(
            'type'  => 'modify',
            'link'  => $modify_path,
            'class' => kkb_button_classer($board_id)
        );
    ...
    if($board_read_under == 'T'){
        if (file_exists(TEMPLATEPATH . '/' . "kingkongboard/board-read-loop.php")) {
            require_once(TEMPLATEPATH . "/kingkongboard/board-read-loop.php");
        } else {      
            if($skin_path){
                if( file_exists($skin_path['abspath']."kingkongboard/board-read-loop.php") && is_plugin_active($skin_path['core']."/".$skin_path['core'].".php") ){
                    require_once($skin_path['abspath']."kingkongboard/board-read-loop.php");
                } else {
                    require_once(KINGKONGBOARD_ABSPATH."/shortcodes/board-read-loop.php");
                }
            } else {
                require_once(KINGKONGBOARD_ABSPATH."/shortcodes/board-read-loop.php");
            }
