<?php
  if(isset($_GET['id'])){
    ...
    $kkbContent .= '<a href="'.get_the_permalink().kingkongboard_set_prefix().'pageid='.$pageid.'" class="kingkongboard-button">'.__('목록보기', 'kingkongboard').'</a>';

    if($prevListNumber){
      $kkbContent .= '<a href="'.get_the_permalink().kingkongboard_set_prefix().'pageid='.$pageid.'&view=read&id='.$prevListNumber.'" class="kingkongboard-button">'.__('이전글', 'kingkongboard').'</a>';
    }
    if($nextListNumber){
      $kkbContent .= '<a href="'.get_the_permalink().kingkongboard_set_prefix().'pageid='.$pageid.'&view=read&id='.$nextListNumber.'" class="kingkongboard-button">'.__('다음글', 'kingkongboard').'</a>';
    }

    if($permission_write_status == "checked"){
        $kkbContent .= '<a href="'.get_the_permalink().kingkongboard_set_prefix().'pageid='.$pageid.'&view=reply&id='.$entry_id.$parent_prm.'" class="kingkongboard-button">'.__('답글쓰기', 'kingkongboard').'</a>';
    }
    $kkbContent .= '</div>';
    ...
    if(kingkongboard_button_permission_check($board_id, $entry_id)){
        $kkbContent .= '<a href="'.get_the_permalink().kingkongboard_set_prefix().'pageid='.$pageid.'&view=modify&id='.$entry_id.'" class="kingkongboard-button">'.__('글수정', 'kingkongboard').'</a>';
    }


    if($permission_delete_status == "checked"){
        $kkbContent .= '<a class="kingkongboard-button button-entry-delete" data="'.get_the_permalink().kingkongboard_set_prefix().'pageid='.$pageid.'&view=delete&id='.$entry_id.'">'.__('글삭제', 'kingkongboard').'</a>';
    }
    $kkbContent .= '</div>';
    $kkbContent .= '</div>';
