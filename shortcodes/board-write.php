<?php
  ...
  $kkbContent .= '<a href="'.get_the_permalink().'" class="'.kkb_button_classer($board_id).' button-prev">'.__('돌아가기', 'kingkongboard').'</a> <a class="'.kkb_button_classer($board_id).' button-save">'.__('저장하기', 'kingkongboard').'</a><div style="float:right; margin-right:10px; position:relative; opacity:0" class="write-save-loading"><div style="float:left; margin-right:10px">'.__('저장중입니다.잠시만 기다려주세요.', 'kingkongboard').'</div><div style="float:left"><img src="'.KINGKONGBOARD_PLUGINS_URL.'/assets/images/ajax-loader3.gif"></div></div>';
  $kkbContent .= '</div>';
  ...
  $kkbContent .= '<input type="hidden" name="pfx" value="'.kingkongboard_set_prefix().'">';
