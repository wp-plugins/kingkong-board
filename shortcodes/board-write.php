<?php
  ...
  $kkbContent .= '<form id="writeForm" method="post" enctype="multipart/form-data" action="'.KINGKONGBOARD_PLUGINS_URL.'/includes/view.save.php">';

  
  $tableClass = apply_filters('kkb_write_table_class', 'kkb-write-table', $board_id);

  $kkbContent .= '<table id="'.$tableID.'" class="'.$tableClass.'">';

  apply_filters("kingkongboard_write_title_before", $kkbContent);

  $kkbContent .= '<tr>';
  $kkbContent   .= '<th class="kingkongboard-write-title">'.apply_filters('kkb_write_title_text', __('제목', 'kingkongboard'), $board_id).'</th>';
  ...
  if( current_user_can('edit_post', get_the_ID()) ){
    $noticeContent  = '<tr>';
    $noticeContent   .= '<th>'.__('공지사항', 'kingkongboard').'</th>';
    $noticeContent   .= '<td><input type="checkbox" name="entry_notice" value="notice"></td>';
    $noticeContent .= '</tr>';
    $kkbContent .= apply_filters('kkb_write_notice_row', $noticeContent, $board_id);
  }

  apply_filters("kingkongboard_write_secret_before", $kkbContent); 

  $scContent  = '<tr>';
  $scContent   .= '<th>'.__('비밀글', 'kingkongboard').'</th>';
  ($board_must_secret_text != null) ? $secret_description = __('비밀글만 허용 됩니다.', 'kingkongboard') : $secret_description = null;
  $scContent   .= '<td><input type="checkbox" name="entry_secret" '.$board_must_secret_text.'> '.$secret_description.'</td>';
  $scContent .= '</tr>';

  $kkbContent .= apply_filters('kkb_write_secret_row', $scContent, $board_id);
  
  apply_filters("kingkongboard_write_section_before", $kkbContent);

  $sections = apply_filters('kkb_write_section_before', $sections, $board_id);
  ...
  if($sections){
    ...
    $secContent  = '<tr>';
    $secContent   .= '<th>'.__('분류선택', 'kingkongboard').'</th>';
    $secContent   .= '<td>';
    $secContent   .= apply_filters('kkb_write_section', '<select name="entry_section">'.$section_value.'</select>', $board_id);
    $secContent   .= '</td>';
    $secContent .= '</tr>';
  } else {
    $secContent = null;
  ...
  $kkbContent .= apply_filters('kkb_write_section_row', $secContent, $board_id);
  ...
  if($board_file_upload == "T"){
    ...
    $addIcon = apply_filters('kkb_write_attach_add_icon', '<img src="'.KINGKONGBOARD_PLUGINS_URL.'/assets/images/button-add.png" style="width:20px; height:auto">', $board_id);
    $kkbContent   .= '<td style="position:relative"><input type="file" name="entry_file[]"> <div class="attachment-controller">'.$addIcon.'</div></td>';
    $kkbContent .= '</tr>';
  }
  ...
  $kkbContent .= '<input type="hidden" name="page_uri" value="'.add_query_arg(array('view' => 'list'), get_the_permalink()).'">';
  $kkbContent .= '<input type="hidden" name="page_id" value="'.$page.'">';
