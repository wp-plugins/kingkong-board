<?php
  ...
  if($board_id){
    ...
    if($board_captcha == "T"){
      $board_captcha_checked = "checked";
    } else {
      $board_captcha_checked = null;
    }

    if($board_thumb_dp == "display"){
      $thumb_dp_checked = "checked";
    } else {
      $thumb_dp_checked = null;
    }
            ...
            <tr>
              ...
              <td>
                <table>
                  <tr>
                    <td colspan="2">
                      <input type="checkbox" name="kkb_board_captcha" value="T" <?php echo $board_captcha_checked;?>> 자동글 방지(google reCAPTCHA)를 사용합니다.
                    <div class="description-container">
                      <span class="description">사용하지 않으시려면 해제 하시면 됩니다.</span>
                    </div> 
                    </td>
                  </tr>
                  <tr>
                ...
                <div class="description-container">
                  <span class="description">자동글 방지는 구글 reCAPTCHA API 를 사용합니다. 자동글 방지를 사용하기 위해서는 서버 클라이언트 URL 라이브러리(CURL)가 설치되어 있어야만 합니다. Site Key 와 Secret Key 를 생성하기 위해서는 구글 개발자 센터에서 등록하셔야 합니다. <a href="https://www.google.com/recaptcha/admin" target="_blank">구글 reCAPTCHA API KEY 등록 바로가기</a></span>
                </div>    
              </td>
            </tr>
            <tr>
...
<?php
  ...
  if($skin_path){
?>
      <div class="settings-panel-left-bottom">
        <?php do_action('kingkong_board_extend_columns'); ?>
      </div>
<?php
  }
?>
