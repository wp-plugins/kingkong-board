jQuery(document).ready(function(){
	...
	jQuery(".pm-tab-wrapper").find("span").click(function(){
		jQuery(".pm-tab-wrapper").find("span").removeClass("active");
		jQuery(this).addClass("active");
		if(jQuery(this).hasClass("pm-tab-entry")){
			jQuery(".pm-tab-wrapper").parent().find(".tab-entry").show();
			jQuery(".pm-tab-wrapper").parent().find(".tab-comment").hide();
		}
		if(jQuery(this).hasClass("pm-tab-comment")){
			jQuery(".pm-tab-wrapper").parent().find(".tab-entry").hide();
			jQuery(".pm-tab-wrapper").parent().find(".tab-comment").show();
		}		
	});
	...
	jQuery(".button-entry-thumbnail-upload").click(function(){
		var duplicated = 0;
		wp.media.editor.send.attachment = function(props, attachment){
			jQuery(".entry-thumbnail-list").html('<div class="entry-attachment-div" data="'+attachment.id+'">'+attachment.filename+'<input type="hidden" name="entry_thumbnail" value="'+attachment.id+'"><div class="entry-thumbnail-remove"></div></div>');
...
function kingkongboard_entry_remove_button_enable(){
	...
	jQuery(".entry-thumbnail-remove").click(function(){
		jQuery(this).parent().css("background", "yellow");
		jQuery(this).parent().animate({opacity: 0}, {duration:500, complete:function(){
			jQuery(this).remove();
		}});
