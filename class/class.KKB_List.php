<?php
    ...
    public function kkb_get_basic_list($page){
      ...
      $filters = "WHERE board_id = '".$config->bid."' AND type = 0 order by list_number ASC ".$Limit;
...
*/
    ...
    public function kkb_get_search_list($post, $page){
      ...
      if(isset($post['kkb_search_keyword'])){
        $results    = $wpdb->get_results("SELECT ID FROM ".$wpdb->posts." WHERE (post_type = '".$config->board_slug."' AND post_status = 'publish') AND (post_title like '%".$post['kkb_search_keyword']."%' or post_content like '%".$post['kkb_search_keyword']."%')");
        $search_id  = array();
        ...
        if(count($results) > 0 ){
          ...
          if(isset($post['kkb_search_section'])){
            if($post['kkb_search_section'] != "all"){
              $filters = "WHERE board_id = '".$config->bid."' AND type = 0 AND section = '".$post['kkb_search_section']."' AND post_id IN (".$search_array.") order by list_number ASC ".$Limit;
            } else {
              $filters = "WHERE board_id = '".$config->bid."' AND type = 0 AND post_id IN (".$search_array.") order by list_number ASC ".$Limit;          
            }
          } else {
            $filters = "WHERE board_id = '".$config->bid."' AND type = 0 AND post_id IN (".$search_array.") order by list_number ASC ".$Limit;          
          }
        ...
        if(isset($post['kkb_search_section'])){
          if($post['kkb_search_section'] != "all"){
            $filters = "WHERE board_id = '".$config->bid."' AND type = 0 AND section = '".$post['kkb_search_section']."' order by list_number ASC ".$Limit;
          } else {
            $filters = "WHERE board_id = '".$config->bid."' AND type = 0 order by list_number ASC ".$Limit;        
          }
        } else {
    ...
    public function kkb_get_search_count($post){
      ...
      $config = $this->config;
      if(isset($post['kkb_search_keyword'])){
        $results    = $wpdb->get_results("SELECT ID FROM ".$wpdb->posts." WHERE post_type = '".$config->board_slug."' AND (post_title like '%".$post['kkb_search_keyword']."%' or post_content like '%".$post['kkb_search_keyword']."%')");
        $search_id  = array();
        foreach($results as $result){
          $search_id[] = $result->ID;
        }
        ...
        if($results){
          ...
          if(isset($post['kkb_search_section'])){
            if($post['kkb_search_section'] != "all"){
              $filters = "WHERE board_id = '".$config->bid."' AND type = 0 AND section = '".$post['kkb_search_section']."' AND post_id IN ($search_array) order by list_number ASC";
            } else {
        ...
        if(isset($post['kkb_search_section'])){
          if($post['kkb_search_section'] != "all"){
            $filters = "WHERE board_id = '".$config->bid."' AND type = 0 AND section = '".$post['kkb_search_section']."' order by list_number ASC ";
          } else {
