<?php
  ...
  class KingkongBoard_Entries {
    ...
    public function Kingkong_Board_Modify_Entry($POST){
      ...
      $entry_content  = kingkongboard_xssfilter(kingkongboard_htmlclear($entry_content));
    ...
    public function Kingkong_Board_Insert_Meta($entry_id, $mktime, $options){
      ...
      if(  current_user_can('manage_options') && isset($options['entry_ymd']) && isset($options['entry_h']) && isset($options['entry_i']) && isset($options['entry_s']) ){
        $guid = $options['entry_guid'];
      } else {
