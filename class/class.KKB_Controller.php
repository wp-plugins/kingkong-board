...
*/
  public function kkb_entry_attach_upload($entry_id, $data, $files){
      ...
      if($files['thumbnail_file']){
        ...
        $dateY = date('Y');
        $dateM = date('m');
        $upload_path  = ABSPATH.'wp-content/uploads/kingkongboard/'.$dateY.'/'.$dateM.'/'.$board_id.'/thumbnail';
        ...
        for ($i=0; $i < count($thumbnail); $i++) { 
          if($thumbnail[$i]['name']){
            $upfile       = $this->kkb_upload_filter($thumbnail[$i]);
      ...
      if($files['entry_file']){
        ...
        $dateY = date('Y');
        $dateM = date('m');
        $upload_path  = ABSPATH.'wp-content/uploads/kingkongboard/'.$dateY.'/'.$dateM.'/'.$board_id.'/attached';
        ...
        for ($i=0; $i < count($Files); $i++) { 
          if($Files[$i]['name']){
            $upfile       = $this->kkb_upload_filter($Files[$i]);
            $file_path    = $upload_path.'/'.$upfile['name'];
