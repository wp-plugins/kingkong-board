  ...
  class KingkongBoard_Initialize {
    ...
    public function kkb_entry_redirect(){
      if (is_single()) {
          global $wpdb, $post;
          //check by postID
          $all_kingkongboards = $this->Get_All_KingkongBoards();
          if (in_array($post->post_type, $all_kingkongboards)) {
            if(strpos($post->post_type, "kkb_") !== false){
              ...
              if(strpos($guid_path, '/?') !== false){
                $prefix = "&";
              } else {
                $prefix = "?";
              }
              wp_redirect( (get_the_permalink($guid).$prefix."view=read&id=".$post->ID) );
              exit();
            }
          }
      }
