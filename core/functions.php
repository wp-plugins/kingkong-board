...
*/
  ...
  function kkb_loop_column_section($content, $entry, $entry_id, $cnt){
    ...
    if($board_sections){
      if($section != "0"){
        $content = "<td class='kingkongboard-list-".$entry['value']."'>".apply_filters('kkb_list_section', kingkongboard_text_cut($section, 5, "..."), $entry_id)."</td>";
      } else {
  ...
  function kkb_loop_column_title($content, $entry, $entry_id, $cnt){
    ...
    if($entry_attachment){
      $entry_attach_text = apply_filters('kkb_list_icon_attach_outer', '<div style="display:inline-block; padding:0; margin:0">'.apply_filters('kkb_list_icon_attach', "<img src='".KINGKONGBOARD_PLUGINS_URL."/assets/images/icon-attachment.png' style='width:15px; height:auto; position:relative; top:0px'>", $entry_id ).'</div>', $entry_id);
    } else {
      $entry_attach_text = "";
    }

    if($entry_secret){
      $entry_secret_icon = apply_filters('kkb_list_icon_secret_outer', '<div style="display:inline-block; padding:0; margin:0">'.apply_filters('kkb_list_icon_secret', '<img src="'.KINGKONGBOARD_PLUGINS_URL.'/assets/images/icon-locked.png" style="width:auto; height:15px !important; position:relative; top:0px; margin-left:5px">', $entry_id).'</div>', $entry_id);
    } else {
      $entry_secret_icon = '';
    }
    ...
    if($entry_depth > 1){
      ...
      $reply_icon     = apply_filters('kkb_list_icon_reply', '<img src="'.KINGKONGBOARD_PLUGINS_URL.'/assets/images/icon-reply.gif" style="width:25px; height:auto; position:relative; top:0px; margin-right:5px">', $entry_id );
