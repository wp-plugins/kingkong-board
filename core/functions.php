    ...
    foreach($comments as $comment){
      if($comment->comment_parent == 0){
        if($comment->comment_approved == 1){
          $cmContent .= '<div class="each-comment">';
          $cmContent .= '<div class="each-comment-author">';
          $cmContent .= '<table class="comment-author-table">';
          $cmContent .= '<tr>';
          if($comment_options['thumbnail'] == "T"){
            $cmContent .= '<td width="30px">';
            $cmContent .= get_avatar($comment->comment_author_email, '20');
            $cmContent .= '</td>';
          }
          $cmContent .= '<td style="font-weight:'.$comment_options['writer']['font_weight'].'; font-size:'.$comment_options['writer']['font_size'].'; color:'.$comment_options['writer']['color'].'">';
          $cmContent .= $comment->comment_author;
          $cmContent .= '</td>';
          $cmContent .= '<td class="comment-author-date" style="color:'.$comment_options['date']['color'].'">';
          $cmContent .= get_comment_date( $comment_options['date']['format'], $comment->comment_ID);
          $cmContent .= '</td>';
          $cmContent .= '<td style="position:relative">';
          $cmContent .= '<img src="'.KINGKONGBOARD_PLUGINS_URL.'/assets/images/icon-comment-reply.png" class="icon-comment-reply">';
          $cmContent .= '<a class="comment-reply-button" data="'.$comment->comment_ID.'">'.__('답글', 'kingkongboard').'</a>';
          $cmContent .= '</td>';
          $cmContent .= '</tr>';
          $cmContent .= '</table>';
          $cmContent .= '</div>';
          $cmContent .= '<div class="each-comment-content" style="color:'.$comment_options['content']['color'].'">';
          $cmContent .= nl2br($comment->comment_content);
          $cmContent .= '</div>';
          $cmContent .= '</div>';
        }
      }
      ...
      if($subComments){
        foreach($subComments as $subcomment){
          if($subcomment->comment_approved == 1){
            $cmContent .= '<div class="each-comment" style="margin-left:30px; position:relative">';
            $cmContent .= '<img src="'.KINGKONGBOARD_PLUGINS_URL.'/assets/images/icon-comment-reply.png" class="kkb_comment_reply_icon" style="top:6px">';
            $cmContent .= '<div class="each-comment-author">';
            $cmContent .= '<table class="comment-author-table">';
            $cmContent .= '<tr>';
            if($comment_options['thumbnail'] == "T"){
              $cmContent .= '<td width="30px">';
              $cmContent .= get_avatar($subcomment->comment_author_email, '20');
              $cmContent .= '</td>';
            }
            $cmContent .= '<td style="font-weight:'.$comment_options['writer']['font_weight'].'; font-size:'.$comment_options['writer']['font_size'].'; color:'.$comment_options['writer']['color'].'">';
            $cmContent .= $subcomment->comment_author;
            $cmContent .= '</td>';
            $cmContent .= '<td class="comment-author-date" style="color:'.$comment_options['date']['color'].'">';
            $cmContent .= get_comment_date( $comment_options['date']['format'], $subcomment->comment_ID);
            $cmContent .= '</td>';
            $cmContent .= '<td style="position:relative">';
            $cmContent .= '<img src="'.KINGKONGBOARD_PLUGINS_URL.'/assets/images/icon-comment-reply.png" class="icon-comment-reply">';
            $cmContent .= '<a class="comment-reply-button" data="'.$comment->comment_ID.'">'.__('답글', 'kingkongboard').'</a>';
            ...
            $cmContent .= '</div>';
            $cmContent .= '<div class="each-comment-content" style="color:'.$comment_options['content']['color'].'">';
            $cmContent .= nl2br($subcomment->comment_content);
            $cmContent .= '</div>';
            $cmContent .= '</div>';
          }
        }      
  ...
  function kingkong_board_manage_entry_column($entries, $entry_id, $cnt){
    ...
    foreach($entries as $entry){
      ...
      switch($entry['value']){
        ...
        $args = array('post_id' => $entry_id, 'count' => true);
        $comments = get_comments($args);
        ...
        if($entry_attachment){
          $entry_attach_text = apply_filters('kingkongboard_entry_attachment_icon', "<img src='".KINGKONGBOARD_PLUGINS_URL."/assets/images/icon-attachment.png' style='width:15px; height:auto; position:relative; top:3px'>" );
        } else {
          $entry_attach_text = "";
        }

        if($entry_secret){
          $entry_secret_icon = '<img src="'.KINGKONGBOARD_PLUGINS_URL.'/assets/images/icon-locked.png" style="width:12px; height:auto; position:relative; top:-2px; margin-left:5px">';
        } else {
          $entry_secret_icon = '';
        }
        ...
        if(get_option('permalink_structure')){
          ...
          $return_content .= "<td style='position:relative' class='kingkongboard-list-".$entry['value']."' ".$reply_padding.">".$reply_icon.$thumbnail."<a href='".get_the_permalink().$prm_pfx."pageid=".$pageid."&view=read&id=".$entry_id.$parent_id."'>".get_the_title($entry_id)."</a> ".$comments_count.$entry_secret_icon." ".$entry_attach_text.$mobile_writer_info."</td>";
        break;
