<?php
  ...
  function kkb_comment_delete(){
    ...
    if( ($user_id == $comment->user_id) && ($comment->user_id != 0) ){
      ...
      $result['status'] = 'failed';
      $result['message'] = __('비회원이거나 본인 글이 아닙니다.', 'kingkongboard');
    }
  ...
  function kingkongboard_password_verifying(){
    ...
    if($input_pwd == $entry_password){
      ...
      if($view == "delete"){
        ...
        $result['redirect'] = apply_filters('kkb_verify_success_redirect', get_the_permalink($guid));
      }
    } else {
      $result['status'] = "failed";
    }
...
*/
  ...
  function kkb_loop_column_number($content, $entry, $entry_id, $cnt){
    ...
    if($entry_type == 1){
      $content = "<td class='kingkongboard-list-".$entry['value']."'>".apply_filters('kkb_list_icon_notice', __('공지', 'kingkongboard'), $entry_id)."</td>";
    } else if($view == 'read' && $entry_id == $_GET['id']){
      $content = "<td class='kingkongboard-list-".$entry['value']."'>".apply_filters('kkb_list_icon_current', __('현재글', 'kingkongboard'), $entry_id)."</td>";
    } else {
  ...
  function kkb_loop_column_title($content, $entry, $entry_id, $cnt){
    ...
    if($entry_depth > 1){
      ...
      (($view == 'read') && $entry_id == $_GET['id']) ? $link = get_the_title($entry_id) : $link = "<a href='".$read_path."'>".get_the_title($entry_id)."</a> ";
      
      $content = "<td style='position:relative;".$reply_padding." ".$view_style."' class='kingkongboard-list-".$entry['value']."'>".apply_filters('kkb_loop_title_value', $reply_icon.$thumbnail.$link.$extra_priority.$mobile_writer_info, $entry_id)."</td>";
  ...
  function get_post_id_by_list_number($board_id, $list_number){
    global $wpdb;
    $board = new KKB_Config($board_id);
    $result = $wpdb->get_row("SELECT post_id FROM ".$board->meta_table." WHERE list_number = '".$list_number."' AND board_id = $board_id AND type != 99");
    if($result){
