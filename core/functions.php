...
if(!is_multisite()){
  add_filter( 'map_meta_cap', 'kingkongboard_map_meta_cap', 10, 4 );
}
function kingkongboard_map_meta_cap( $caps, $cap, $user_id, $args ) {
  
}

function kingkongboard_add_skin($type, $slug, $title, $plugins_url, $abspath){
  if(!$type || !$slug || !$title){
    return false;
  } else {
    $duplicate = 0;
