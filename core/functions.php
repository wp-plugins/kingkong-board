  ...
  function kingkong_board_manage_entry_column($entries, $entry_id, $cnt){
    ...
    foreach($entries as $entry){
      ...
      switch($entry['value']){
        ...
        if($comments_count > 0){
          $comments_count = " <span class='entry_comment_count'>[<strong>".$comments_count."</strong>]</span>";
        } else {
          $comments_count = null;
        }

        if($entry_attachment){
          $entry_attach_text = '<div style="display:inline-block; padding:0; margin:0">'.apply_filters('kingkongboard_entry_attachment_icon', "<img src='".KINGKONGBOARD_PLUGINS_URL."/assets/images/icon-attachment.png' style='width:15px; height:auto; position:relative; top:0px'>" ).'</div>';
        } else {
          $entry_attach_text = "";
        }

        if($entry_secret){
          $entry_secret_icon = '<div style="display:inline"><img src="'.KINGKONGBOARD_PLUGINS_URL.'/assets/images/icon-locked.png" style="width:auto; height:15px !important; position:relative; top:0px; margin-left:5px"></div>';
        } else {
          $entry_secret_icon = '';
        }
