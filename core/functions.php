      ...
      switch($entry['value']){
        ...
        case "number" :
          if($entry_type == 1){
            ...
            $return_content .= "<td class='kingkongboard-list-".$entry['value']."'>".apply_filters('kkb_list_number', $cnt, $entry_id)."</td>";
          }
        break;
        ...
        if($board_sections){
          ...
          if($section != "0"){
            $return_content .= "<td class='kingkongboard-list-".$entry['value']."'>".apply_filters('kkb_list_section', kingkongboard_text_cut($section, 5, "..."), $entry_id)."</td>";
          } else {
        ...
        if($thumbnail_image){
          ...
          if($board_thumbnail_dp == "display"){ 
            $thumbnail = '<div class="entry_thumbnail_image">'.apply_filters('kkb_list_thumbnail', $thumbnail_image, $entry_id).'</div>';
          } else {
            $thumbnail = "";
          }
        } else {
        ...
        if($comments_count > 0){
          $comments_count = apply_filters('kkb_list_comment_count', " <span class='entry_comment_count'>[<strong>".$comments_count."</strong>]</span>", $entry_id);
        } else {
          $comments_count = null;
        }

        if($entry_attachment){
          $entry_attach_text = '<div style="display:inline-block; padding:0; margin:0">'.apply_filters('kingkongboard_entry_attachment_icon', "<img src='".KINGKONGBOARD_PLUGINS_URL."/assets/images/icon-attachment.png' style='width:15px; height:auto; position:relative; top:0px'>", $entry_id ).'</div>';
        } else {
          $entry_attach_text = "";
        }

        if($entry_secret){
          $entry_secret_icon = '<div style="display:inline">'.apply_filters('kkb_list_icon_secret', '<img src="'.KINGKONGBOARD_PLUGINS_URL.'/assets/images/icon-locked.png" style="width:auto; height:15px !important; position:relative; top:0px; margin-left:5px">', $entry_id).'</div>';
        } else {
          $entry_secret_icon = '';
        }
        ...
        if($entry_depth > 1){
          ...
          $reply_icon     = apply_filters('kkb_list_icon_reply', '<img src="'.KINGKONGBOARD_PLUGINS_URL.'/assets/images/icon-reply.gif" style="width:25px; height:auto; position:relative; top:0px; margin-right:5px">');
          ...
          $return_content .= "<td style='position:relative;".$reply_padding."' class='kingkongboard-list-".$entry['value']."'>".$reply_icon.$thumbnail."<a href='".$read_path."'>".get_the_title($entry_id)."</a> ".$comments_count.$entry_secret_icon." ".$entry_attach_text.$mobile_writer_info."</td>";
        break;

        case "date" :
          $return_content .= "<td class='kingkongboard-list-".$entry['value']."'>".apply_filters('kkb_list_date', get_the_date('Y.m.d', $entry_id), $entry_id)."</td>";
        break;
        ...
        case "writer" :
          ...
          $return_content .= "<td class='kingkongboard-list-".$entry['value']."'>".apply_filters('kkb_list_writer', $writer, $entry_id)."</td>";
        break;
